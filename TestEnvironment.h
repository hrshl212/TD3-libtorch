...
{
    Eigen::Vector2d pos_; //[[x], [y]] one column two rows
    Eigen::Vector2d goal_;
    Eigen::VectorXd state_; //dynamic vector with type double

    double old_dist_;

	//the initailizer list is basically assigning size
	//note that pos_ is a class in itself whose constructor take in size value as parameter
    TestEnvironment(double x, double y) : goal_(2), pos_(2), state_(4)
    {
        goal_ << x, y;
        pos_.setZero(); //Intializing position to 0,0
        state_ << pos_, goal_;  
    ...
    auto Reward(int status) -> torch::Tensor
    {
        ...
        switch (status)
        {
            ...
            case WON:
                reward[0][0] += 10.;
                printf("won, reward: %f", reward[0][0].item<double>());
                break;
            case LOST:
                reward[0][0] -= 10.;
                printf("lost, reward: %f", reward[0][0].item<double>());
                break;
        }
