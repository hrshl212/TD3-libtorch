...
int main() {
    ...
    VT states;	//VT = std::vector<torch::Tensor>
    VT actions;
    VT rewards;
    VT dones;
    ...
    for (uint e=1;e<=n_epochs;e++)
    {
        ...
        for (uint i=0;i<n_iter;i++)
        {
            ...
            values.push_back(std::get<1>(av)); //seesm to be v_value
            ...
            if (c%n_steps == 0)
            {
                ...
                values.push_back(std::get<1>(ac->forward(states[c-1]))); //seems to be taking the v values
